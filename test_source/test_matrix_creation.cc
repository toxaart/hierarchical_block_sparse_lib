...
static int test_creation() {
	...
	M.set_n_block_multiplicaitons(3);
  ...
  verify_that_matrices_are_equal<MatrixType>(M,B);
	assert(M.get_n_block_multiplications() == B.get_n_block_multiplications());
  ...
  verify_that_matrices_are_equal<MatrixType>(B,C);
	assert(C.get_n_block_multiplications() == B.get_n_block_multiplications());
